...
router.get('/getEmailUser/:email', (req, res) => {
...
router.post('/updateUserProfile', encoder, (req, res) => {
    const namad = req.body.nama
    const namab = req.body.nama_belakang
    const email = req.body.email

    connection.query('UPDATE user SET nama = ?, nama_belakang = ? WHERE email = ?', [namad, namab, email], (error, result, fields) => {
        if (error) {
            console.log(error)
        } else {
            console.log("Success")
...
router.get('/getOnePlanes/:urutan', (req, res) => {
    const uR = req.params.urutan
    connection.query('SELECT * FROM pesawat where urutan = ?', [uR], (error, rows) => {
        if (error) {
            console.log(error);
            res.status = 300;
        } else {
...
router.get('/getPlane/:id_pesawat', encoder, (req, res) => {
...
router.post('/insertPlane', encoder, (req, res) => {
    ...
    const no_duduk = req.body.r
    const gate = req.body.t
    const jam_terbang = req.body.y
